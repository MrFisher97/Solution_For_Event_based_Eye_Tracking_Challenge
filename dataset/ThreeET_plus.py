...
class ThreeETplus_Eyetracking(tonic.Dataset):
...
class MyDataset(ThreeETplus_Eyetracking):
    # get item with dataset name
    def __getitem__(self, index: int) -> Tuple[Any]:
        data_name = self.data[index].split('/')[-1].split('.')[0]
        events, target = super().__getitem__(index)
        return events, target, data_name

class MySlicedDataset(tonic.SlicedDataset):
    def generate_metadata(self):
        ...
        return [
            ...
            for data, targets, _ in self.dataset
        ]

    def __getitem__(self, item) -> Any:
        dataset_index, slice_index = self.slice_dataset_map[item]
        data, targets, data_name  = self.dataset[dataset_index]
        ...
        return data_slice, target_slice, data_name, slice_index == (len(self.metadata[dataset_index]) - 1)