...
class MC_Loss(nn.Module):
    ...
    def forward(self, inputs, targets):
        targets = targets[..., :2]
        loss = (((inputs - targets) * self.weights) ** 2).sum(dim=2).sqrt().mean()
        
        m_input = (inputs[:, :-1] - inputs[:, 1:]) * self.weights
        m_target = (targets[:, :-1] - targets[:, 1:]) * self.weights
        loss += 1e-2 * (((m_input - m_target)) ** 2).sum(dim=2).sqrt().mean()
        return loss